...
use DG\OpenticketBundle\DataFixtures\ORM\TicketCategoriesFixtureLoader;
 ...
 */
class TicketCategoriesFixtureLoaderTest extends \PHPUnit_Framework_TestCase
{
    /**
     * @var TicketCategoriesFixtureLoader
     */
    private $fixture;
    ...
    protected function setUp()
    {
        ...
        $this->fixture = new TicketCategoriesFixtureLoader($this->fixtureDataMock, $this->fixtureLoadCheckerMock, $this->managerMock, $eventDispatcherMock);
    }
    ...
    public function testLoadNoDuplicate()
    {
        ...
        $this->managerMock->expects($this->at(0))->method('persist')->willReturnCallback(function (Category $category) {
            $this->setCategoryId($category, 1);
        });
        $this->managerMock->expects($this->at(1))->method('flush');
        $this->managerMock->expects($this->at(2))->method('persist')->with(Translation::create()->setLocale('en')->setTranslation('Test1')->setDomain('ticket_category')->setKey(1));
        $this->managerMock->expects($this->at(3))->method('persist')->with(Translation::create()->setLocale('ru')->setTranslation('Тест1')->setDomain('ticket_category')->setKey(1));
    ...
    private function setCategoryId(Category $category, \int $id)
    {
